  ...
  loadData$: Observable<Action> = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action: LoadData) => {
        ...
        const longTermId = `${action.payload.longTermStartTime}`;
        const currentUser = action.payload.currentUser;

        return [
          new StreamQuarter([['__id', '==', longTermId]], {}, loadId),
          new StreamQuarterGoal([['__userId', '==', currentUser.__id], ['__quarterId', '==', longTermId]], {}, loadId),
        ];
      }),
    ),
  );
